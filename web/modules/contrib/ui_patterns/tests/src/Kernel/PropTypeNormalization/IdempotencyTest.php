...
final class IdempotencyTest extends KernelTestBase {
  ...
  public function testNormalizeIsIdempotent(): void {
    $manager = \Drupal::service(PropTypePluginManager::class);
    $cases = $this->cases();
    $checked = 0;
    ...
    foreach (\array_keys($manager->getDefinitions()) as $id) {
      ...
      self::assertInstanceOf(PropTypeInterface::class, $instance);
  ...
  private function cases(): array {
    $component = \Drupal::service(ComponentPluginManager::class)->find('ui_patterns_test:test-component');
