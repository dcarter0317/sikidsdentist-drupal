 ...
 *
 * @FieldFormatter(
 *   id = "glightbox_responsive",
 *   module = "glightbox",
 *   label = @Translation("GLightbox Responsive"),
 *   field_types = {
 *     "image",
 *   },
 * )
 */
class GLightboxResponsiveFormatter extends ImageFormatterBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * GLightbox attachment object of the output.
   *
  ...
  /**
   * Constructs a GLightboxResponsiveFormatter object.
   *
  ...
  public static function defaultSettings() {
    return [
      'glightbox_responsive_node_style' => '',
      'glightbox_image_style' => '',
      'glightbox_responsive_image_style' => '',
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $element['glightbox_responsive_node_style'] = [
      '#title' => $this->t('Responsive image style for content'),
      '#type' => 'select',
      '#default_value' => $this->getSetting('glightbox_responsive_node_style'),
      '#required' => TRUE,
    ...
    $element['glightbox_responsive_image_style'] = [
      '#title' => $this->t('Responsive image style for GLightbox'),
      '#type' => 'select',
      '#default_value' => $this->getSetting('glightbox_responsive_image_style'),
      '#required' => TRUE,
      '#options' => $responsive_image_options,
      '#description' => $responsive_image_admin_link->toRenderable() + [
        '#access' => $this->currentUser->hasPermission('administer responsive image styles'),
  ...
  public function settingsSummary() {
    ...
    if (isset($responsive_image_styles[$this->getSetting('glightbox_responsive_node_style')])) {
      $summary[] = $this->t('Responsive content image style: @style', ['@style' => $responsive_image_styles[$this->getSetting('glightbox_responsive_node_style')]->label()]);
    }
    else {
    ...
    if (isset($responsive_image_styles[$this->getSetting('glightbox_responsive_image_style')])) {
      $summary[] = $this->t('GLightbox responsive image style: @style', ['@style' => $responsive_image_styles[$this->getSetting('glightbox_responsive_image_style')]->label()]);
    }
    else {
      $summary[] = $this->t('GLightbox responsive image style: Original image');
    }
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    $responsive_image_style = $this->responsiveImageStyleStorage->load($this->getSetting('glightbox_responsive_node_style'));
    ...
    foreach ($files as $delta => $file) {
      $settings['style_name'] = $settings['glightbox_responsive_node_style'];
      ...
      $elements[$delta] = [
        '#theme' => 'glightbox_responsive_formatter',
        '#item' => $item,
    ...
    if ($this->attachment->isApplicable()) {
      ...
      $dompurify = $this->libraryDiscovery->getLibraryByName('glightbox', 'dompurify');
      ...
      if ($dompurify_exists) {
        $elements['#attached']['library'][] = 'glightbox/dompurify';
      }
    }
  ...
  public function calculateDependencies() {
    ...
    $style_ids = [];
    $style_ids[] = $this->getSetting('glightbox_responsive_node_style');
    $style_ids[] = $this->getSetting('glightbox_responsive_image_style');
