...
class GenerateCSS {
  ...
  public function generateFromSettings($settings, $block_class) {
    ...
    if (!empty($settings['other_settings']['border_color'])) {
      ...
      $border_color = EbtConstants::COLOR_BLACK;
    }
      ...
      if (!empty($media) && $media->getSource()->getPluginId() == 'image') {
        ...
        if (!empty($media_url)) {
          ...
          if (!empty($settings['other_settings']['background_image_settings']['addOverlay'])) {
            ...
            $global_styles .= '.' . $block_class . '::before { content: ""; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background-color: rgba(' . $r . ', ' . $g . ', ' . $b . ', ' . $overlay_alpha . '); pointer-events: none; z-index: 0; } ';
            $global_styles .= '.' . $block_class . ' .ebt-container { position: relative; } ';
          }
        }
      }
    }
